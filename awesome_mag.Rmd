...
mean((pred.elast - y_test)^2) # 0.419
...
mean((pred.lasso - y_test)^2) # 0.419
pred.pcr <- predict(pcr.fit$finalModel, newdata = x_test, type = "response")
mean((pred.pcr - y_test)^2) # 0.450
pred.pls <- predict(pls.fit$finalModel, newdata = x_test, type = "response")
mean((pred.pls - y_test)^2) # 0.410
pred.lm <- predict(lm.fit, newdata = data.frame(x_test))
mean((pred.lm - y_test)^2) # 0.41??
pred.ridge <- predict(ridge.fit, newdata = x_test)
