...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, out.width = 0.9, messages = FALSE)
library(tidyverse)
library(caret)
library(glmnet)
library(mgcv)
...
```{r, eval=FALSE}
library(MASS)
mult.fit1 <- lm(user_rating ~ size_megabytes + price + user_rating_ver + cont_rating + prime_genre + sup_devices_num + ipad_sc_urls_num + lang_num, data = apple) 
boxcox(mult.fit1) 
```
