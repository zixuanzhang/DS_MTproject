...
apple <- read_csv("./data/AppleStore.csv") %>%
  ...
  mutate(prime_genre = as.integer(ifelse(prime_genre == "Games", 1, 0)),
         user_rating = as.factor(user_rating)) %>% 
...
```{r}
set.seed(1234)
bagging <- ranger(user_rating~., train_data1, mtry = 8, min.node.size = 4) 
...
```{r}
set.seed(1234)
rf2.final.per <- ranger(user_rating~., train_data1,
...
```{r}
set.seed(1234)
rf.final.imp <- ranger(user_rating~., train_data1,
                        mtry = 2, splitrule = "variance",
                        min.node.size = 6,
                        importance = "impurity")
