...
# Try more
gbm.grid <- expand.grid(n.trees = c(2000, 3000, 4000), interaction.depth = 1:6,
...
```{r}
rpart.pred.label <- predict(rpart.fit, test_data)
table(rpart.pred.label, test_data)
rpart.pred.label2 <- predict(rpart.fit2, test_data1) 
table(rpart.pred.label, test_data1)
rf.pred.label <- predict(rf.fit, newdata = test_data) 
table(rf.pred.label, test_data)
rf.pred.label2 <- predict(rf.fit2, newdata = test_data1) 
table(rf.pred.label2, test_data)
gbmB.pred.label <- predict(gbm.fit, newdata = test_data) 
table(gbmB.pred.label, test_data)
gbmB.pred.label2 <- predict(gbm.fit2, newdata = test_data1) 
table(gbmB.pred.label2, test_data)
```
...
plot(roc.gbm, add = TRUE, col = 5) 

auc <- c(roc.rpart$auc[1], roc.rpartc$auc[1], roc.rf$auc[1], roc.gbm$auc[1])
modelNames <- c("rpart","ctree","rf","gbm") 
