...
#Bagging
```{r}
bag.grid <- expand.grid(mtry = 8, splitrule = "variance", min.node.size = 1:80) 
set.seed(1234)
...
```{r}
set.seed(1234)
bagging <- ranger(user_rating~., train_data1, mtry = 8, min.node.size = 59) 
...
```{r}
# Try more
gbm.grid2 <- expand.grid(n.trees = c(2000, 3000, 5000), interaction.depth = 1:10,
                        shrinkage = c(0.001,0.003,0.005), n.minobsinnode = 1)
set.seed(1234)
