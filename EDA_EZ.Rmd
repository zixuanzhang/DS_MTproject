...
lasso.fit$bestTune # 0.0094 almost no penalty
...
plot(enet.fit) 
```
comment: shows alpha = 1 (lasso) gives the lowest RMSE
...
```
comment: lasso seems to outperform the other two. We should use the lasso model. The RMSE is similar to cv result from glmnet package

selected model: lasso 
```{r}
lasso.fit$bestTune # 0.011
predict(lasso.fit$finalModel, s=lasso.fit$bestTune$lambda, type="coefficients")
```
