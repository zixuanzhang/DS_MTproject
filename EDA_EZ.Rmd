...
# MSE
sum((y_test - pred.lasso)^2) / length(y_test) # 0.83
```

fit linear model
```{r}
lm.model <- lm(user_rating ~., data = apple)
summary(lm.model)
pred.lm <- predict(lm.model, newdata = data.frame(x_test), type = "response")
dim(pred.lm)
names(data.frame(x_test))
sum((y_test - pred.lm)^2) / length(y_test)
```
...
test MSE
```{r}
sum((y_test - predict(ridge.fit$finalModel, s=ridge.fit$bestTune$lambda, newx = x_test,
          type="response"))^2) / length(y_test) # 0.84
sum((y_test - predict(lasso.fit$finalModel, s=lasso.fit$bestTune$lambda, newx = x_test,
          type="response"))^2) / length(y_test) # 0.83
sum((y_test - predict(lm.fit, newdata = x_test))^2) / length(y_test) # 0.82
```
