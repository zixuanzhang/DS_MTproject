...
apple <- apple %>% 
  select(-c(x1, track_name, id, currency, ver, rating_count_tot, rating_count_ver)) %>% 
...
hist(apple_train$size_bytes) # right skew
hist(apple_train$size_bytes[apple_train$size_bytes < 1000]) # right skewed
sum(apple_train$size_bytes < 1000) # 5213/5399 = 96% 

hist(apple_train$price)
hist(apple_train$price[apple_train$price < 10])
sum(apple_train$price < 10) # 5328/5399 = 99%

hist(apple_train$user_rating) # response

hist(apple_train$user_rating_ver) 

hist(apple_train$sup_devices_num)
hist(apple_train$ipad_sc_urls_num)
unique(apple_train$ipad_sc_urls_num) # 0,1,2,3,4,5

hist(apple_train$lang_num)

table(apple_train$cont_rating) # categorical
table(apple_train$prime_genre) # categorical
unique(apple_train$prime_genre)
table(apple_train$vpp_lic) # binary
```
...
featurePlot(apple_train[, -c(3,5,6)], apple_train$user_rating, plot = "scatter", labels = c("","Y"),
...
```{r}
apple_train <- apple_train %>% select(user_rating, everything())
summary(apple)

appleCor <-  cor(apple_train[-c(5,6)]) # correlation matrix of the modified dataset above
appleCor
...
corrplot::corrplot(cor(x_train), method = "circle",tl.cex = 0.5)
```
...
```{r}
nzv <- nearZeroVar(apple_train) # nzv: near zero variance
apple_train <- apple_train[, -nzv]
combInfo <- findLinearCombos(apple_train[,-c(5,6)]) # on numerical values
combInfo # no linear dependency problem
names(apple_train) # remove vpp_lic
```

##Box-Cox Transformation
```{r}
apple = apple %>% 
  mutate(user_rating_new = user_rating + 0.01)
mult.fit1 <- lm(user_rating_new ~ size_bytes + price + user_rating_ver + cont_rating + prime_genre + sup_devices_num + ipad_sc_urls_num + lang_num + vpp_lic, data = apple) 
boxcox(mult.fit1) 
```
