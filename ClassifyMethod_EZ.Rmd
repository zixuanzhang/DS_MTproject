...
km <- kmeans(dat1, centers = 6, nstart = 20) # try 20 different initial values

km_vis <- fviz_cluster(list(data = dat1, cluster = km$cluster),  
                       ellipse.type = "convex", # boundary shapes
                       geom = c("point","text"), # add pokemon names
                       labelsize = 5, 
...
```{r}
fviz_dend(hc.complete, k = 6,   # make cut at forming 4 clusters    
          cex = 0.3, 
          palette = "jco", 
